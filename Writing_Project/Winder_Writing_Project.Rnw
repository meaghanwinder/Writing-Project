...
<<eDNA_summary, include = F, warning = F, message = F>>=
# BOM
...
\begin{figure}[]
<<eDNA_visualization_temp, echo = F, warning = F, message = F, fig.width = 9.5, fig.height = 4.5, fig.align = 'center'>>=
# visualization of eDNA data
eDNA.df %>% 
  ggplot(aes(x = Site, 
             y = Water.Temp, 
  ...
  labs(title = 'Water Temperature by Site', 
       x = 'Site', 
       y = expression(paste('Water Temperatuere (', degree, 'C)')), 
       color = 'Date') +
  geom_point() + 
  scale_colour_manual(values = date.cols) + 
  theme_bw() + 
...
<<load_tow_data, include = F, warning = F, message = F>>=
# load tow data
tow <- read_excel(
  "C:/Users/mwind/OneDrive/Writing Project_EXTRA/USBR_2018_data_sample.xlsx",
  sheet = 3)
