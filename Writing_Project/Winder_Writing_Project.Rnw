...
Since the number of positive droplets per sample is based on the arbitrary choice of an amplitude to distinguish between positive and negative droplet, for this project, the droplet results will be aggregated to a positive or negative result at the sample level. \hl{\texit{If Alison sends over the droplet information and I can get a plot of the amplitudes I'll insert that here and discuss it.}} To limit the possibility of false positive samples for these analyses, to be considered a positive sample, the recommended threshold of three or more positive droplets will be used. The top plot in Figure \ref{fig:eDNA_sample} displays the proportion of positive samples by site if all samples with positive droplets are treated as positive samples, and the bottom plot displays the proportion of positive samples by site using the above threshold to limit the positive sites to those with three or more positive droplets. Figure \ref{fig:eDNA_sample} can be used to visualize the changes in the proportion of positive samples from each site when the threshold is used. The use of the threshold changes the proportion of positive samples in six of the twelve sites; the proportion of positive samples are: 0.5333 for site BOM1, 0.9333 for site BOM2, 0.8666 for lake BOM3, 0.0000 for sites LG1, LG2, LG4, and LG5, 0.1000 for site LG2,  and finally 0.0000 for all sites in lake MG.
...
## number of postive samples in each site
site.detect3 <- eDNA %>% 
  group_by(Site) %>%
...
## proportions of positive samples in each site
...
\begin{figure}[]
<<eDNA_visualization_sample, echo = F, warning = F, message = F, fig.width = 9.5, fig.height = 7.6, fig.align = 'center'>>=
...
prop.plot <- eDNA.df2 %>% 
  ggplot(aes(x = Site, 
             y = Prop)) + 
  geom_point() +
  labs(title = 'Proportion of Positive Samples by Site when all Samples with Positive Droplets are Considered Positive', 
       x = 'Site', 
...
prop3.plot <- eDNA.df2 %>% 
  ggplot(aes(x = Site, 
             y = Prop3)) + 
  geom_point() +
  labs(title = 'Proportion of Positive Samples by Site when the Threshold of 3 or more Positive Droplets is Used', 
       x = 'Site', 
       y = 'Proportion of Positive Samples') +
  ylim(c(0, 1)) +
  theme_bw() + 
...
<<eDNA_data_format, warning = F, message = F>>=
...
post_summary(m1.psi.mcmc, plot.options = labs(y = "psi"))
...
post_summary(m1.p.mcmc, plot.options = labs(y = "p"))
...
post_summary(m2.p.mcmc, plot.options = labs(y = "p"))
...
post_summary(m3.p.mcmc)


@

<<eDNA_mod4, include = F>>=
# model psi ~ site, p ~ water.temp + date

## Priors 
beta_m <- matrix(0, nrow = 12, ncol = 1) # prior mean for beta
sigma_inv_beta <- diag(nrow(beta_m))/1000 # prior covariance for beta

alpha_m <- matrix(0, nrow = 6, ncol = 1) # prior mean for alpha
sigma_inv_alpha <- diag(nrow(alpha_m))/1000 # prior covariance for alpha


## fit the model 
m4 <- PGocc4(formula = V1 ~ site ~ as.numeric(W1) + W2, 
             design_mats = data, 
             ndraws = 50000, 
             alpha_m = alpha_m, 
             beta_m = beta_m, 
...
m4.p.mcmc <- exp(model.matrix(~as.numeric(data$W$W1) + data$W$W2) %*% m4.alpha)/(1 + exp(model.matrix(~as.numeric(data$W$W1) + data$W$W2) %*% m4.alpha))
...
<<eDNA_mod5, include = F>>=
# model psi ~ site, p ~ water.temp + date + site

## Priors 
beta_m <- matrix(0, nrow = 12, ncol = 1) # prior mean for beta
sigma_inv_beta <- diag(nrow(beta_m))/1000 # prior covariance for beta

alpha_m <- matrix(0, nrow = 17, ncol = 1) # prior mean for alpha
sigma_inv_alpha <- diag(nrow(alpha_m))/1000 # prior covariance for alpha


## fit the model 
m5 <- PGocc4(formula = V1 ~ site ~ as.numeric(W1) + W2 + W3, 
             design_mats = data, 
             ndraws = 50000, 
             alpha_m = alpha_m, 
             beta_m = beta_m, 
...
post_summary(m5.beta, 'beta')

m5.psi.mcmc <- exp(model.matrix(~data$X$site) %*% m5.beta)/(1 + exp(model.matrix(~data$X$site) %*% m5.beta))
...
m5.p.mcmc <- exp(model.matrix(~as.numeric(data$W$W1) + data$W$W2 + data$W$W3) %*% m5.alpha)/(1 + exp(model.matrix(~as.numeric(data$W$W1) + data$W$W2 + data$W$W3) %*% m5.alpha))
...
<<eDNA_mod6, include = F>>=
# model psi ~ site, p ~ water.temp * lake + date

## Priors 
beta_m <- matrix(0, nrow = 12, ncol = 1) # prior mean for beta
sigma_inv_beta <- diag(nrow(beta_m))/1000 # prior covariance for beta

alpha_m <- matrix(0, nrow = 10, ncol = 1) # prior mean for alpha
sigma_inv_alpha <- diag(nrow(alpha_m))/1000 # prior covariance for alpha


## fit the model 
m6 <- PGocc4(formula = V1 ~ site ~ as.numeric(W1) * W4 + W2, 
             design_mats = data, 
             ndraws = 50000, 
             alpha_m = alpha_m, 
             beta_m = beta_m, 
...
m6.p.mcmc <- exp(model.matrix(~as.numeric(data$W$W1) * data$W$W4 + data$W$W2) %*% m6.alpha)/(1 + exp(model.matrix(~as.numeric(data$W$W1) * data$W$W4 + data$W$W2) %*% m6.alpha))
