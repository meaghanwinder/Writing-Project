...
eDNA data description
\begin{itemize}
  \item data source: part of larger study involving Asian clams 
  \item region: north-eastern United States, VT and NY
  \item number of lakes: 3 with data, 4 total... the last one did not have any positive replicates
  \item 3 sites in Lake Bomoseen, 5 sites in Lake George, 5 sites (one inaccessible) in Lake Memphremagog
  \item 5 samples (at all sites) and a field blank (at most sites)
  \item 20000 ddPCR replicates from each sample??
  \item data collected summer/fall 2019
  \item water temperature information available
\end{itemize}
...
\begin{alltt}
\hlcom{# put the code chunk names here like this: <<name>>}

\hlstd{eDNA} \hlkwb{<-} \hlkwd{read.csv}\hlstd{(}\hlstr{"C:/Users/mwind/OneDrive/Writing Project_EXTRA/eDNA.csv"}\hlstd{)}
\hlkwd{levels}\hlstd{(eDNA}\hlopt{$}\hlstd{Lake)}
\end{alltt}
\end{kframe}
\end{knitrout}
